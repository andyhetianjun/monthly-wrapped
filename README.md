...
button and the Spotify player are automatically left out of the export.
...
- **Monthly stats:** top 5 artists and songs, top decade, and unique-artist
  count for your last ~4 weeks.
- **Auto theme from your top song:** the opening colour theme is derived from
  your #1 track's album art, then cycle to 5 hand-picked themes with the arrows.
- **Live album-art backdrop:** a blurred wall of your top covers behind the card.
- **Inline Spotify player:** preview or play your top track without leaving the page.
...
- The access token lasts about 1 hour. When it expires, API calls return 401,
  and the app clears the token and sends you back to the login screen.
- After login the token is handed to the browser and kept in local storage, so
  each visitor is independent.
